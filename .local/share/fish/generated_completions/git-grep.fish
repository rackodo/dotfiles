# git-grep
# Autogenerated from man page /opt/homebrew/share/man/man1/git-grep.1
...
complete -c git-grep -l no-index -d 'Search files in the current directory that is not managed by Git, or by ignor…'
...
complete -c git-grep -l threads -d 'Number of grep worker threads to use.  See NOTES ON THREADS and grep'
