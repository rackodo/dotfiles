# ri
# Autogenerated from man page /usr/share/man/man1/ri.1
complete -c ri -s i
complete -c ri -s a
complete -c ri -s l
complete -c ri -s T -d 'Synonym for -no-pager'
complete -c ri -s w
complete -c ri -l width -d 'Set the width of the output'
complete -c ri -l server -d 'Run RDoc server on the given port.  The default port is\\~8214'
complete -c ri -s f
complete -c ri -l format -d 'Use the selected formatter'
complete -c ri -s h
complete -c ri -l help -d 'Show help and exit'
complete -c ri -s v
complete -c ri -l version -d 'Output version information and exit'
complete -c ri -s d
complete -c ri -l doc-dir -d 'List of directories from which to source documentation in addition to the sta…'
complete -c ri -l no-standard-docs -d 'Do not include documentation from the Ruby standard library, site_lib, instal…'
complete -c ri -l dump -d 'Dump data from an ri cache or data file'

