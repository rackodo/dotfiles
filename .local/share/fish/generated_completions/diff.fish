...
complete -c diff -s A -d 'Configure the algorithm used when comparing files'
...
complete -c diff -l changed-group-format -d 'Format input groups in the provided the format is a string with special keywo…'
complete -c diff -l ignore-file-name-case -d 'ignore case when comparing file names'
complete -c diff -l no-ignore-file-name-case -d 'do not ignore case when comparing file names (default)'
...
complete -c diff -l tabsize -d 'Number of spaces representing a tab (default 8) El Directory comparison optio…'
