...
complete -c tcpdump -l apple-oneline -d 'PD Do not insert line break after IP header in verbose mode for easier parsing'
...
complete -c tcpdump -l apple-md-print -d 'PD Control the display of packet metadata via an optional  metadata_arg argum…'
...
complete -c tcpdump -l apple-pcapng -d 'PD Use the pcap-ng file format when saving files.  This is an Apple addition'
...
complete -c tcpdump -l apple-md-filter -d 'PD See the PACKET METADATA FILTER section below.  This is an Apple addition'
